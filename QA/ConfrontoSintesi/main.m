...
X0_3 = zeros(1, 12)+1;
P_3 = points_set(1:3, :);
[X3_sol, F3, exitflag_3, output_3] = fsolve(@(X) solve_fun(X, P_3, A, D, L1, L2), X0_3, options);
time_3 = toc;

residuo_3 = norm(F3, inf);
...
[X5_sol, F5, exitflag_5, output_5] = fsolve(@(X) solve_fun(X, P_5, A, D, [], []), X0_5, options);
time_5 = toc;

residuo_5 = norm(F5, inf);

% --- Caso 7 punti ---
tic;
X0_7 = zeros(1, 28)+1; % Servono 2 parametri noti
P_7 = points_set(1:7, :);
[X7_sol, F7, exitflag_7, output_7] = fsolve(@(X) solve_fun(X, P_7, A, [], [], []), X0_7, options);
time_7 = toc;

residuo_7 = norm(F7, inf);

% --- Caso 9 punti ---
tic;
X0_9 = zeros(1, 36)+1; % Non servono parametri noti
P_9 = points_set(1:9, :);
[X9_sol, F9, exitflag_9, output_9] = fsolve(@(X) solve_fun(X, P_9, [], [], [], []), X0_9, options);
time_9 = toc;

residuo_9 = norm(F9, inf);
...
show_comparison(QA_3, QA_5, QA_7, QA_9, F3, F5, F7, F9, exitflag_3, exitflag_5, exitflag_7, exitflag_9, time_3, time_5, time_7, time_9)